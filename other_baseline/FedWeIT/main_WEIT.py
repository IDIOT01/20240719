...
from data_manager import Next_stream_data
...
if __name__ == "__main__":
    ...
    if (
        "cifar" in args.dataset
        or args.dataset == "mnist"
        or "miniimagenet" in args.dataset
        or "FC100" in args.dataset
        or "Corn50" in args.dataset
    ):
        ...
        for c in dataset_train.keys():
            dataset_train[c]["y"] = list(
                np.asarray(dataset_train[c]["y"]).astype("int64")
            )
            dataset_test[c]["y"] = list(
                np.asarray(dataset_test[c]["y"]).astype("int64")
            )
    ...
    for name, para in net_glob.named_parameters():
        if "aw" in name:
            shape = np.concatenate(
                [para.shape, [int(round(args.num_users * args.frac))]], axis=0
            )
    ...
    for iter in range(args.epochs):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            #                             idxs=dict_users_train, indd=indd)
            # else:
            tr_loader, test_loader = Next_stream_data(dataset_train[task], args, idx)
            tr_dataloaders = tr_loader  # DataLoader(DatasetSplit(dataset_train[task],dict_users_train[idx][:args.m_ft]),batch_size=args.local_bs, shuffle=True)
            w_local = []
            appr = apprs[idx]
            appr.set_sw(w_agg)
            ...
            if iter % args.round == args.round - 1:
                ...
                for aw in aws:
                    shape = np.concatenate(
                        [aw.shape, [int(round(args.num_users * args.frac))]], axis=0
                    )
                    from_kb_l = np.zeros(shape)
                    if len(shape) == 5:
        ...
        if iter % args.round == args.round - 1:
            for i in range(args.num_users):
                ...
                if len(apprs[i].pre_weight["aw"]) < task + 1:
                    ...
                    tr_dataloaders = tr_loader  # DataLoader(DatasetSplit(dataset_train[task], dict_users_train[i][:args.m_ft]),batch_size=args.local_bs, shuffle=True)
            ...
            acc_test, loss_test = test_img_local_all_WEIT(
                apprs,
                args,
                dataset_test,
                dict_users_test,
            ...
            if iter != args.epochs:
                print(
                    "Round {:3d}, Train loss: {:.3f}, Test loss: {:.3f}, Test accuracy: {:.2f}".format(
                ...
                print(
                    "Final Round, Train loss: {:.3f}, Test loss: {:.3f}, Test accuracy: {:.2f}".format(
