...
def test_img_local(net_g,dataload_test, dataset, args,t,idx=None,indd=None, user_idx=-1, idxs=None,appr = None):
    net_g.eval()
    test_loss = 0
    correct = 0
    ...
    if leaf:
        data_loader = dataload_test#DataLoader(DatasetSplit_leaf(datatest_new,np.ones(len(datatest_new))), batch_size=args.local_bs, shuffle=False)
    else:
        data_loader = dataload_test#DataLoader(DatasetSplit(dataset,idxs), batch_size=args.local_bs,shuffle=False)
...
def test_img_local_all_WEIT(appr, args,dataload_test, dataset_test, dict_users_test, t, w_locals=None, w_glob_keys=None, indd=None,
    ...
    for idx in range(num_idxxs):
        ...
        for u in range(t + 1):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                a, b = test_img_local(net_local,dataload_test, dataset_test, args, t, idx=dict_users_test[idx], indd=indd,
                                      user_idx=idx)
                # tot += len(dataset_test[dict_users_test[idx]]['x'])
            else:
                a, b = test_img_local(net_local,dataload_test , dataset_test[u], args, u, user_idx=idx, idxs=dict_users_test[idx])
                all_task_acc += a
                all_task_loss += b
        all_task_acc /= (t + 1)
        all_task_loss /= (t + 1)
        if 'femnist' in args.dataset or 'sent140' in args.dataset:
            tot += len(dataload_test.dataset)
        else:
            tot += len(dataload_test.dataset)
        if 'femnist' in args.dataset or 'sent140' in args.dataset:
            acc_test_local[idx] = all_task_acc * len(dataload_test.dataset)
            loss_test_local[idx] = all_task_acc * len(dataload_test.dataset)
        else:
            acc_test_local[idx] = all_task_acc * len(dataload_test.dataset)
            loss_test_local[idx] = all_task_loss * len(dataload_test.dataset)
        del net_local
